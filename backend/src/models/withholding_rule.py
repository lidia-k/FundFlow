...
class WithholdingRule(Base):
    ...
    state_code = Column(
        SQLEnum(USJurisdiction), nullable=False
    )  # From "State Abbrev" column
    entity_type = Column(
        String(50), nullable=False
    )  # Entity type coding (Partnership, Corporation, etc.)
    ...
    income_threshold = Column(
        DECIMAL(12, 2), nullable=False
    )  # From "Per Partner Income Threshold"
    tax_threshold = Column(
        DECIMAL(12, 2), nullable=False
    )  # From "Per Partner W/H Tax Threshold"
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "tax_threshold >= 0.00", name="ck_withholding_rule_tax_threshold_positive"
        ),
