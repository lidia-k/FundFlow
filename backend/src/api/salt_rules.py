...
class UploadResponse(BaseModel):
    ...
    rule_counts: dict[str, int] | None = Field(None, alias="ruleCounts")
...
async def upload_salt_rules(
    ...
    if description and len(description) > 500:
        raise HTTPException(
            status_code=400, detail="Description must be 500 characters or less"
        )
    ...
    if not file.filename.lower().endswith((".xlsx", ".xlsm")):
        raise HTTPException(
            status_code=400, detail="File must be Excel format (.xlsx or .xlsm)"
        )
    ...
    try:
        ...
        try:
            ...
            if existing_active_rule_set:
                logger.info(
                    f"Found existing active rule set: {existing_active_rule_set.id}, archiving it"
                )
            ...
            if storage_result.error_message:
                raise HTTPException(
                    status_code=400, detail=storage_result.error_message
                )
            ...
            response = UploadResponse(
                ...
                uploaded_file={
                    ...
                    "uploadTimestamp": storage_result.source_file.upload_timestamp.isoformat()
                    + "Z",
                },
...
async def get_rule_set_detail(
    ...
    include_rules: bool = Query(
        False, description="Include actual rule data in response"
    ),
    db: Session = Depends(get_db),
) -> dict[str, Any]:
...
async def delete_rule_set(
    rule_set_id: str, db: Session = Depends(get_db)
) -> dict[str, Any]:
